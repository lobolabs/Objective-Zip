...
typedef struct ourmemory_s {
  const void *base; /* Base of the region of memory we're using */
   ...
   int mode;
{
     ...
     * architectures
     */
    if (sscanf(filename,"%p+%zX",&mem->base,&mem->size)!=2)
      return NULL;
   ...
   uLong size;
{
    ...
    memcpy((void *)mem->base + mem->cur_offset, buf, size);
    mem->cur_offset+=size;
   ...
   int origin;
{
    ...
    if (new_pos > mem->limit)
      memset((void *)mem->base + mem->limit, 0, new_pos - mem->limit);
